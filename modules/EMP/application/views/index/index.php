...
<body>
                   ...
                   <div id="table">
                        <div class="box">
                            ...
                            <form name="login">
                                ...
                                <a href="#" id="frgt-pwd" >Forget password?</a>
    ...
    $('#frgt-pwd').click(function(){
       var frgt = document.forms['login'];
       var mail = frgt.elements['email'].value;
       var mailfld = frgt.elements['email'];
       if(mail == ''){
           $(mailfld).addClass('month-leavs_err').val('').focus();
                  $(mailfld).effect('shake', {
                times: 3,
                distance: 5
            }, 300);
           $("#login").find(".login_err").css("opacity", "1").html("Please enter your email address to reset your password");
           return;
       }else{
       $.ajax({
          url: 'index/frgtpwd',
          ...
          success: function(res){
              if(res == -1){
                  var err = 'Email Id you entered is Invalid. Please try again.';
                  $(mailfld).addClass('month-leavs_err').val('').focus();
                  $(mailfld).effect('shake', {
                times: 3,
                distance: 5
            }, 300);
                 $("#login").find(".login_err").css("opacity", "1").html(err);
              }else{
                  $("#login").find(".login_err").css("opacity", "1").html("<i class='icon-ok' style='color: green'></i>"+res+"");
              }

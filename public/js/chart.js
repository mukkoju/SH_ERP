...
function barchart(d, colnum, w, h, chartId, clr)
{
  ...
  var trgt = $('#' + chartId);
  $("#" + chartId).children(".chart-tags").attr('id', "barchart-tags");
  //Previous heading removed and new heading added(purpose: when there are radio buttons)
  $("#" + chartId).children("#chart-head").remove();
  $("#" + chartId).children("#legend").remove();
  $("#" + chartId).children("#yaxis").remove();
  ...
  function infiniteBarChange() {
    trgt.find(".col-name").prop("checked", false);
    trgt.find(".col-name:eq(" + col_num + ")").prop("checked", true);
    trgt.find(".col-name:eq(" + col_num + ")").trigger("change");
    if (col_num >= clms.length - 1)
      col_num = 0;
  }
  ...
  function updateBar(clnm) {
    ...
    trgt.find(".tick").children("text").each(function() {
      var v = $(this).text();
  ...
  //on hover effects
  trgt.find('.svg').children("g").on("mouseenter", function(e)
  {
  ...
  trgt.find('.svg').children("g").on("mouseleave", function()
  {
    barChartMouseLeave($(this).attr('id'), chartId);
  });
  trgt.find('.svg2').children("g").on("mouseenter", function(e)
  {
    xaxisMouseEnter($(this).attr('val'), e, chartId);
  });
  trgt.find('.svg2').children("g").on("mouseleave", function()
  {
...
//    r = r * 1.15; //this is for some top padding   
  //id created for chart and chart tags divisions
  var trgt = $('#' + chartId);
  $("#" + chartId).children(".chart-tags").attr('id', "linechart-tags");
  //Previous heading removed and new heading added(purpose: when there are radio buttons)
  $("#" + chartId).children("#chart-head").remove();
  $("#" + chartId).children("#legend").remove();
  $("#" + chartId).children("#yaxis").remove();
...
function groupedbarchart(d, w, h, chartId, title)
{
  ...
  {
    $("#" + chartId).append("<p class = 'txtbx vtbx'></p>");
    var hvrbx = $("#" + chartId).children(".txtbx");
    hvrbx.html(numberFormat($(this).attr("v")));
    $("#" + chartId).children(".txtbx").css({position: 'absolute'});
    var left = parseInt($(this).attr('x')) + ($(this).attr("width") / 2);

    var width = numberFormat($(this).attr("v")).length * 10;
    if ((width + left) > $("#" + chartId).outerWidth()) {
      hvrbx.addClass("v-end");
      hvrbx.css({top: parseInt($(this).attr('y')) - 24 + $(this).parents(".chart").find(".arrows").outerHeight(), right: ($("#" + chartId).outerWidth() - left - 8), width: width});
    }
    else {
      left = parseInt($(this).attr('x')) - (hvrbx.outerWidth() / 2) + ($(this).attr("width") / 2);
      hvrbx.removeClass("v-end");
      hvrbx.css({top: parseInt($(this).attr('y')) - 24 + $(this).parents(".chart").find(".arrows").outerHeight(), left: left});
    }
...
function groupedlinechart(d, w, h, chartId, title)
{
  ...
  //id created for chart and chart tags divisions
  var trgt = $('#' + chartId);
  $("#" + chartId).children(".chart-tags").remove();
  //$("#" + chartId).children(".chart-tags").attr('id', "linechart-tags");
  //Previous heading removed and new heading added(purpose: when there are radio buttons)
  $("#" + chartId).children("#chart-head").remove();
  $("#" + chartId).children("#legend").remove();
  ...
  trgt.find('.svg').find(".circle").on("mouseenter", function(e)
  {
    if (e != 0) {
      ...
      var width = numberFormat($(this).attr("v")).length * 10;
      if ((width + relativeX) > $("#" + chartId).outerWidth()) {
        hvrbx.addClass("v-end");
        hvrbx.css({top: relativeY, right: ($("#" + chartId).outerWidth() - relativeX - 8), width: width});
      }
      else {
...
//      ;
  ...
  trgt.children(".chart-tags").attr('id', "piechart-tags");
  ...
  var svg = d3.select("#" + chartId)
          ...
          .attr("height", h)
          .attr("width", w);
  trgt.children("#chart-head").empty();
  ...
  function updatePie(clnm) {
    ...
    $("#" + chartId + ">.svg").children("g").on("mouseenter", function(e)
    {
      pieChartMouseEnter($(this).attr('id'), e, data, chartId, title);
    });
    $("#" + chartId + ">.svg").children("g").on("mousemove", function(e)
    {
      pieChartMouseEnter($(this).attr('id'), e, data, chartId, title);
    });
    $("#" + chartId + ">.svg").children("g").on("mouseleave", function()
    {
...
//    else {
//      trgt.children("#chart-head").remove();
//      trgt.children('#cht-opts').after('<p id = "chart-head">No data</p>');
//      trgt.children(".chart-tags").empty();
//    }
  function arcTween(a) {
...
function buildYaxis(mxn, mxp, zeroth_pos, chartId, h, w, leftmargin, no_lbls, lbl_padding, ch, bpdng) {
  ...
  Yaxis.append("g")
          ...
          .attr("transform", "translate(0 ,0)")
          .call(yAxis);
  $(".tick").children("text").attr("y", -7);
  $(".tick").children("text").attr("x", 4);
  $(".tick").children("line").attr("x1", 6);
  $(".tick").children("line").attr("x2", w + leftmargin);
...
function showTags(data, chart, chartId, title, colnum) {
  var trgt = $('#' + chartId);
  trgt.find(".chart-tags").remove();
  trgt.append("<ul class='chart-tags'></ul>");
...
function barChartMouseEnter(v, x, h, e, id, chartId, w) {
  if (e != 0) {
    ...
    if ((width + left) > $("#" + chartId).outerWidth()) {
      hvrbx.addClass("v-end");
      hvrbx.css({bottom: parseInt(h) + 30, right: ($("#" + chartId).outerWidth() - left - 8), width: width});
    }
    else {
  ...
  $("#" + chartId + ">.svg").find("#" + id + "").find(".r").css("opacity", "0.9");
}
function barChartMouseLeave(id, chartId) {
  $("#" + chartId + ">.svg").find("#" + id + "").find(".r").css("opacity", "1");
  $("#" + chartId + ">.svg").find("#" + id + "").find(".bottom-label").css("fill", "#404244").attr('class', 'transition in')
          .css("font-size", "1em");
  $("#" + chartId).children(".txtbx").remove();
}
function lineChartMouseEnter(v, e, id, chartId) {
  if (e != 0) {
    ...
    if ((width + relativeX) > $("#" + chartId).outerWidth()) {
      ...
      $("#" + chartId).find(".txtbx").css({top: relativeY, right: ($("#" + chartId).outerWidth() - relativeX - 8), width: width});
    }
    else {
      relativeX = relativeX - (hvrbx.outerWidth() / 2);
  ...
  $("#" + chartId + ">.svg").find("#" + id + "").find(".circle").css("opacity", "0.8");
  $("#" + chartId + ">.svg").find("#" + id + "").find(".circle").attr("r", 8);
}
function lineChartMouseLeave(id, chartId) {
  $("#" + chartId + ">.svg").find("#" + id).find(".circle").css("opacity", "1");
  $("#" + chartId + ">.svg").find("#" + id).find(".circle").attr("r", 5);
...
function pieChartMouseEnter(id, e, data, chartId, title) {
  ...
  if (e != 0) {
    ...
    {
      if (data[0].length == 1)
        var v = $("#" + chartId + ">.svg").children("#" + id).attr("value") + " " + data[0][0];
      else
        v = $("#" + chartId + ">.svg").children("#" + id).attr("value");
      trgt.append("<p class = 'txtbx pie-bx'></p>");
      trgt.children(".txtbx").html(v);
      trgt.children(".txtbx").css({top: relativeY, left: relativeX, position: 'absolute'});
    }
  }
  $("#" + chartId + ">.svg").find("#" + id).css("opacity", "0.9");
  $("#" + chartId + ">.svg").find("#" + id).find("text").css("fill", "white").attr('class', 'transition in');
}
function pieChartMouseLeave(id, chartId) {
  var trgt = $("#" + chartId);
  trgt.children(".txtbx").remove();
  $("#" + chartId + ">.svg").find("#" + id + "").css("opacity", "1");
  $("#" + chartId + ">.svg").find("#" + id + "").find("text").css("fill", "white")
          .css("font-size", "1em");
}
function xaxisMouseEnter(hovertext, e, chartId) {
  if (e != 0) {
    ...
    $("#" + chartId).append("<p class = 'txtbx'></p>");
    if ((width + relativeX) > $("#" + chartId).outerWidth()) {
      $("#" + chartId).find(".txtbx").addClass("x-end");
      $("#" + chartId).find(".txtbx").css({top: relativeY, right: ($("#" + chartId).outerWidth() - relativeX + 36), position: 'absolute', width: width});
    }
    else {

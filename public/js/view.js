...
//                $('.procss-btn').show();
                ...
                for (var i = 0; i < arry_length; i++) {
                  var str = "<tr id='data-tr'><td>";
                    ...
                    if (d[i]['_emp_stmnts_status'] == 1)
                        str += "<td class='stats_td'><button class='btn btn-success pay_cnfrm' id='pay_cnfrm' value='Done' type='button'>Done</button><button class='btn btn-danger pay_cncl' id='pay_cncl' value='Cancel' type='button' style='color: #FF7171;'>Cancel</button></td>";
                    else if (d[i]['_emp_stmnts_status'] == 2)
                        str += "<td class='paid-img stats_td'><div class='paid-smbl'><div class='paid-smbl-innr'><p>PAID</p></div></div><div class='revrt'>REVERT</div></td>";
                    else
    ...
    $('.main-content').on('click', '.pay_cnfrm', function(){
        ...
        obj = {
            ...
            success: function (res) {
                ...
                alert(res);
                var $this = $(this);
                    $('#table1').find($this).parent().html("<div class='paid-smbl'><div class='paid-smbl-innr'><p>PAID</p></div></div><div class='revrt'>REVERT</div>");
//                $("#resp-popup").find(".popupBody").html(res);
//                $("#btn-trgr").trigger('click');
                setTimeout(function () {
                    window.location.reload();
    ...
    $(this).find('.revrt').css({'top': '0'});
    });
    $('.main-content').on('mouseleave', '.paid-img', function(){
    $(this).find('.revrt').css({'top': '100px'});
    });
...
//    alert($('nav ul').find('li [data-pg="'+p+'"]').attr('href'));
        var mn = $('nav ul').find('li a[data-pg="'+p+'"]');
...
//       $(this).after('<span class="bar"></span>');
    ...
    $('.main-content').on('click', 'button', function (e) {
        var $this = $(this);
...
//    $('input[type=checkbox]').after("<span class=ripple></span><span class=check></span>");
    ...
    $('.main-content').on('click', '.stmnts-expnd', function(){
        if($('.bnk-stmnts-tbl').height() == 1){
            ...
            $('.stmnts-expnd i:nth-child(2)').removeClass('mdi-navigation-expand-less').addClass('mdi-navigation-expand-more');
            $('.bnk-stmnts-tbl').css({'opacity':'0', 'height': '0'}); 
        }
    });
    
    // @download page collapsemore and collapseless
    $('.main-content').on('click', '.dwnld-clps', function(e){
        var hght = $(this).height();
        if(hght > 40){
        if($(event.target).prop("class")== 'clps-actv'){
            $(this).find('h2').removeClass('clps-actv');
            $(this).children().css({'opacity': '1'});
            $(this).css({'height': '39px', 'border-top-left-radius': '4px', 'border-top-right-radius': '4px', 'overflow': 'hidden'});
            $(this).find('h2 i').removeClass('mdi-navigation-expand-less').addClass('mdi-navigation-expand-more');
        }
        }else{
        $(this).find('h2').addClass('clps-actv');
        $(this).find('h2 i').removeClass('mdi-navigation-expand-more').addClass('mdi-navigation-expand-less');
        $(this).children().css({'opacity': '1'});
        $(this).css({'height': '200', 'border-top-left-radius': '4px', 'border-top-right-radius': '4px'});
        $(this).siblings('.dwnld-clps').css({'height': '39px', 'overflow': 'hidden', 'border-top-left-radius': '0px', 'border-top-right-radius': '0px'});
        $(this).siblings('.dwnld-clps').find('h2').removeClass('clps-actv');
        $(this).siblings('.dwnld-clps').find('h2 i').removeClass('mdi-navigation-expand-less').addClass('mdi-navigation-expand-more');
    ...
    $('body').on('click', '.cntnt', function(e){
        ...
        $.ajax({
            ...
            success: function(d){
                ...
                var p = location.pathname.split("/")[1];
                var mn = $('nav ul').find('li a[data-pg="'+p+'"]');
                mn.parent('li').addClass('mnu-activ').siblings().removeClass('mnu-activ');
            }
        });
